...
def search_news_smart(topics, selected_media, check_embassy):
    ...
    if check_embassy:
        ...
        embassy_queries = [
            "site:za.china-embassy.gov.cn notice",
            "site:johannesburg.china-consulate.gov.cn notice",
            "site:durban.china-consulate.gov.cn notice",
            "site:capetown.china-consulate.gov.cn notice"
        ]
        for q in embassy_queries:
            try:
                # 使馆公告放宽到一个月(m)，确保不错过重要通知
                res_list = list(ddgs.text(keywords=q, region="za-en", timelimit="m", max_results=1))
                for res in res_list:
                    # 简单去重：只收录标题里有 Notice 或 提醒 的
                    results.append({
                        "type": "EMBASSY",
                        "category": "领事提醒",
                        "title": res['title'],
...
def get_history_fun_fact():
    ...
    return fact if fact else "South Africa has 3 capital cities!"
...
def generate_viral_article(news_data, history_fact):
    ...
    prompt = f"""
    You are the Chief Editor of "OONCE South Africa Daily".
    ...
    3. **Structure**:
       ...
       - **Part 2 📰**: General News summary.
       - **Part 3 📜**: History story.
    """
...
with st.sidebar:
    ...
    topics = st.multiselect(
        "话题 (Topics)",
        ["Immigration/Visa", "Crime/Safety", "Joburg Traffic", "Loadshedding", "Rand/Exchange Rate"],
    ...
    check_embassy = st.checkbox("扫描使领馆公告", value=True)
    
    st.markdown("---")
    
    if st.button("🔄 开始全网扫描"):
        with st.spinner("🕵️‍♂️ 正在执行三级搜索策略..."):
            # 1. 智能搜索
            ...
            if results:
                st.success(f"成功抓取 {len(results)} 条资讯！")
            else:
                st.warning("全网搜索均为 0 条结果，可能是 DuckDuckGo 接口暂时被限流，请稍后再试。")
...
if 'scan_results' in st.session_state:
    ...
    st.markdown(f"### 📡 抓取结果 (共 {news_count} 条)")
    
    if news_count > 0:
        with st.expander("📄 点击展开查看抓取详情", expanded=True):
        ...
        st.error("⚠️ 虽然扫描完成，但没有抓到任何新闻。请尝试在左侧增加更多话题，或取消勾选'指定媒体'以扩大范围。")
